-- Update admin function to use Stripe tables instead of custom subscribers table
...
BEGIN
  ...
  -- Generate customer ID if needed
  customer_id := 'cust_admin_' || encode(gen_random_bytes(10), 'hex');
  ...
  ON CONFLICT (email) 
  DO UPDATE SET
    attrs = json_build_object('user_id', target_user_id, 'subscription_tier', p_subscription_tier);
  
  -- If not free tier, create subscription record
  IF p_subscription_tier != 'free' THEN
    subscription_id := 'sub_admin_' || encode(gen_random_bytes(10), 'hex');
