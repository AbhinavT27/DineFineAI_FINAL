-- Update admin function with correct timestamp format
...
BEGIN
  ...
  ON CONFLICT (email) 
  DO UPDATE SET
    attrs = json_build_object('user_id', COALESCE(target_user_id, (EXCLUDED.attrs->>'user_id')::uuid), 'subscription_tier', p_subscription_tier);
  ...
  IF p_subscription_tier != 'free' THEN
    ...
    INSERT INTO public."Subscribers" (
      ...
      customer_id,
      now()::timestamp without time zone,
      p_subscription_end::timestamp without time zone,
      'usd',
      json_build_object(
    ...
    ON CONFLICT (id) 
    DO UPDATE SET
      current_period_end = p_subscription_end::timestamp without time zone,
